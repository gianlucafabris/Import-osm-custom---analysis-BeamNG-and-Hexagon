...
names(raw) = c("time", "position_x", "position_y", "position_z", "speed", "rpm", "position_r", "position_p", "position_h", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null")
names(raw2) = c("time", "throttle", "steering", "cluch", "parkingbrake", "brake", "null")
names(custom) = c("time", "position_x", "position_y", "position_z", "speed", "rpm", "position_r", "position_p", "position_h", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null")
names(custom2) = c("time", "throttle", "steering", "cluch", "parkingbrake", "brake", "null")
names(custom_elevation) = c("time", "position_x", "position_y", "position_z", "speed", "rpm", "position_r", "position_p", "position_h", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null")
names(custom_elevation2) = c("time", "throttle", "steering", "cluch", "parkingbrake", "brake", "null")

raw = raw[, !(names(raw) %in% c("speed", "rpm", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null"))]
raw2 = raw2[, !(names(raw2) %in% c("throttle", "cluch", "parkingbrake", "brake", "null"))]
custom = custom[, !(names(custom) %in% c("speed", "rpm", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null"))]
custom2 = custom2[, !(names(custom2) %in% c("throttle", "cluch", "parkingbrake", "brake", "null"))]
custom_elevation = custom_elevation[, !(names(custom_elevation) %in% c("speed", "rpm", "temp_water", "speed2", "stering_wheel_rotation", "throttle", "brake", "cluch", "speed_air", "speed_airflow", "altitude", "reverse", "throttle2", "null"))]
...
for index, row in r.custom.iterrows():
    ...
    if best_i < l1:
      roadid_custom.append(1)
    elif best_i < l1+l2:
      roadid_custom.append(2)
    else:
      roadid_custom.append(3)
    i = max(best_i - 100, 0)
...
for index, row in r.custom_elevation.iterrows():
    ...
    if best_i < l1:
      roadid_custom_elevation.append(1)
    elif best_i < l1+l2:
      roadid_custom_elevation.append(2)
    else:
      roadid_custom_elevation.append(3)
    i = max(best_i - 100, 0)
...
ggsave("output/acceleration.png", grid.arrange(
  plot_3var(
    ...
    raw$acceleration_x_norm, c((floor(calc_scale(c(raw$acceleration_x_norm, raw$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration x",
    raw$acceleration_y_norm, c((floor(calc_scale(c(raw$acceleration_x_norm, raw$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration y",
  ...
  plot_3var(
    ...
    custom$acceleration_x_norm, c((floor(calc_scale(c(custom$acceleration_x_norm, custom$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration x",
    custom$acceleration_y_norm, c((floor(calc_scale(c(custom$acceleration_x_norm, custom$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration y",
  ...
  plot_3var(
    ...
    custom_elevation$acceleration_x_norm, c((floor(calc_scale(c(custom_elevation$acceleration_x_norm, custom_elevation$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration x",
    custom_elevation$acceleration_y_norm, c((floor(calc_scale(c(custom_elevation$acceleration_x_norm, custom_elevation$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration y",
  ...
  plot_2var(
    raw, "Raw - acceleration x y and z",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    raw$acceleration_x_norm, c((floor(calc_scale(c(raw$acceleration_x_norm, raw$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration planar and spatial"
  ) +
  ...
  plot_2var(
    custom, "Custom - acceleration x y and z",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$acceleration_x_norm, c((floor(calc_scale(c(custom$acceleration_x_norm, custom$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration planar and spatial"
  ) +
  ...
  plot_2var(
    custom_elevation, "Custom elevation - acceleration x y and z",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$acceleration_x_norm, c((floor(calc_scale(c(custom_elevation$acceleration_x_norm, custom_elevation$acceleration_y_norm))[c(1,2)]/10)+1)*10, 10), "acceleration planar and spatial"
  ) +
  ...
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$acceleration_spatial_norm, c(floor(min(raw$acceleration_spatial_norm)), floor(max(raw$acceleration_spatial_norm))+1, 10), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  ...
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$acceleration_spatial_norm, c(floor(min(custom$acceleration_spatial_norm)), floor(max(custom$acceleration_spatial_norm))+1, 10), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  ...
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$acceleration_spatial_norm, c(floor(min(custom_elevation$acceleration_spatial_norm)), floor(max(custom_elevation$acceleration_spatial_norm))+1, 10), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - acceleration planar and spatial",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    raw$acceleration_spatial_norm, c(floor(min(raw$acceleration_spatial_norm)), floor(max(raw$acceleration_spatial_norm))+1, 10), "acceleration planar and spatial"
  ) +
  ...
  plot_2var(
    ...
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$acceleration_spatial_norm, c(floor(min(custom$acceleration_spatial_norm)), floor(max(custom$acceleration_spatial_norm))+1, 10), "acceleration planar and spatial"
  ) +
  ...
  plot_2var(
    ...
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$acceleration_spatial_norm, c(floor(min(custom_elevation$acceleration_spatial_norm)), floor(max(custom_elevation$acceleration_spatial_norm))+1, 10), "acceleration planar and spatial"
  ) +
...
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for dist_planar given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered can be considered equal and also the high p-value (0.48) reinforces the decision to accept the hypothesis that the means are equal.

#so the custom road generator influences the average distance 2D (also the variance), but the elevation doesn't effect the distance 2D.
...
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for dist_spatial given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered can be considered equal and also the high p-value (0.46) reinforces the decision to accept the hypothesis that the means are equal.

#so the custom road generator influences the average distance 3D (also the variance), but the elevation doesn't effect the distance 3D.
...
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for speed_planar given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (7.53e-5) reinforces the decision to decisively reject the hypothesis that the means are equal
...
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for speed_spatial given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (8.36e-5) reinforces the decision to decisively reject the hypothesis that the means are equal
...
#From the output it can be deduced for acceleration_z given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered equal and also the high p-value (0.86) reinforces the decision to accept the hypothesis that the means are equal
#also for acceleration_z given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered equal and also the high p-value (0.84) reinforces the decision to accept the hypothesis that the means are equal
