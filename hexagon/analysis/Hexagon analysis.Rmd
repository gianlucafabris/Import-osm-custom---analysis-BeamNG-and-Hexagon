...
ggsave("output/position z.png", grid.arrange(
  plot_3var(
    raw, "Raw - position x y and z",
    raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
    raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
    raw$position_z-min(raw$position_z), c(0, floor(max(raw$position_z-min(raw$position_z)))+1, 1), "position z"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom, "Custom - position x y and z",
    custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
    custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
    custom$position_z-min(custom$position_z), c(0, floor(max(custom$position_z-min(custom$position_z)))+1, 1), "position z"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom_elevation, "Custom elevation - position x y and z",
    custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
    custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
    custom_elevation$position_z-min(custom_elevation$position_z), c(0, floor(max(custom_elevation$position_z-min(custom_elevation$position_z)))+1, 1), "position z"
  ) +
    geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - z",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    raw$position_z-min(raw$position_z), c(0, floor(max(raw$position_z-min(raw$position_z)))+1, 1), "position z"
  ) +
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom, "Custom - z",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$position_z-min(custom$position_z), c(0, floor(max(custom$position_z-min(custom$position_z)))+1, 1), "position z"
  ) +
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom_elevation, "Custom elevation - z",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$position_z-min(custom_elevation$position_z), c(0, floor(max(custom_elevation$position_z-min(custom_elevation$position_z)))+1, 1), "position z"
  ) +
    geom_line(color="#6B9EE1"),
...
ggsave("output/position rph.png", grid.arrange(
  # plot_3var(
  #   raw, "Raw - position x y and pitch",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$position_p_norm, calc_scale(c(raw$position_p_norm, custom$position_p_norm, custom_elevation$position_p_norm)), "pitch"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and pitch",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$position_p_norm, calc_scale(c(raw$position_p_norm, custom$position_p_norm, custom_elevation$position_p_norm)), "pitch"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and pitch",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$position_p_norm, calc_scale(c(raw$position_p_norm, custom$position_p_norm, custom_elevation$position_p_norm)), "pitch"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   raw, "Raw - position x y and roll",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$position_r_norm, calc_scale(c(raw$position_r_norm, custom$position_r_norm, custom_elevation$position_r_norm)), "roll"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and roll",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$position_r_norm, calc_scale(c(raw$position_rp_norm, custom$position_r_norm, custom_elevation$position_r_norm)), "roll"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and roll",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$position_r_norm, calc_scale(c(raw$position_r_norm, custom$position_r_norm, custom_elevation$position_r_norm)), "roll"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   raw, "Raw - position x y and yaw",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$position_h_norm, c(-180, 180, 45), "yaw"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and yaw",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$position_h_norm, c(-180, 180, 45), "yaw"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and yaw",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$position_h_norm, c(-180, 180, 45), "yaw"
  # ) +
  #   geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - pitch",
  ...
  plot_2var(
    ...
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom_elevation, "Custom elevation - pitch",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$position_p_norm, calc_scale(c(raw$position_p_norm, custom$position_p_norm, custom_elevation$position_p_norm)), "pitch"
  ) +
    geom_line(color="#6B9EE1"),
  ...
  plot_2var(
    custom, "Custom - roll",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$position_r_norm, calc_scale(c(raw$position_r_norm, custom$position_r_norm, custom_elevation$position_r_norm)), "roll"
  ) +
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom_elevation, "Custom elevation - roll",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$position_r_norm, calc_scale(c(raw$position_r_norm, custom$position_r_norm, custom_elevation$position_r_norm)), "roll"
  ) +
    geom_line(color="#6B9EE1"),
  ...
  plot_2var(
    ...
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom_elevation, "Custom elevation - yaw",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$position_h_norm, c(-180, 180, 45), "yaw"
  ) +
    geom_line(color="#6B9EE1"),
...
ggsave("output/speed.png", grid.arrange(
  plot_3var(
    raw, "Raw - position x y and speed spatial",
    raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
    raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
    raw$speed_spatial_norm, c(0, floor(max(raw$speed_spatial_norm))+1, 20), "speed spatial"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom, "Custom - position x y and speed spatial",
    custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
    custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
    custom$speed_spatial_norm, c(0, floor(max(custom$speed_spatial_norm))+1, 20), "speed spatial"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom_elevation, "Custom elevation - position x y and speed spatial",
    custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
    custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
    custom_elevation$speed_spatial_norm, c(0, floor(max(custom_elevation$speed_spatial_norm))+1, 20), "speed spatial"
  ) +
    geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - speed planar and spatial",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    ...
    labs(color = "speed"),
  plot_2var(
    custom, "Custom - speed planar and spatial",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    ...
    labs(color = "speed"),
  plot_2var(
    custom_elevation, "Custom elevation - speed planar and spatial",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
...
ggsave("output/acceleration.png", grid.arrange(
  plot_3var(
    raw, "Raw - acceleration x y and z",
    raw$acceleration_x_norm, c(calc_scale(c(raw$acceleration_x_norm, raw$acceleration_y_norm))[c(1,2)], 1), "acceleration x",
    raw$acceleration_y_norm, c(calc_scale(c(raw$acceleration_x_norm, raw$acceleration_y_norm))[c(1,2)], 1), "acceleration y",
    raw$acceleration_z_norm, c(floor(min(raw$acceleration_z_norm)), floor(max(raw$acceleration_z_norm))+1, 1), "acceleration z"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom, "Custom - acceleration x y and z",
    custom$acceleration_x_norm, c(calc_scale(c(custom$acceleration_x_norm, custom$acceleration_y_norm))[c(1,2)], 1), "acceleration x",
    custom$acceleration_y_norm, c(calc_scale(c(custom$acceleration_x_norm, custom$acceleration_y_norm))[c(1,2)], 1), "acceleration y",
    custom$acceleration_z_norm, c(floor(min(custom$acceleration_z_norm)), floor(max(custom$acceleration_z_norm))+1, 1), "acceleration z"
  ) +
    geom_point(alpha = 1),
  plot_3var(
    custom_elevation, "Custom elevation - acceleration x y and z",
    custom_elevation$acceleration_x_norm, c(calc_scale(c(custom_elevation$acceleration_x_norm, custom_elevation$acceleration_y_norm))[c(1,2)], 1), "acceleration x",
    custom_elevation$acceleration_y_norm, c(calc_scale(c(custom_elevation$acceleration_x_norm, custom_elevation$acceleration_y_norm))[c(1,2)], 1), "acceleration y",
    custom_elevation$acceleration_z_norm, c(floor(min(custom_elevation$acceleration_z_norm)), floor(max(custom_elevation$acceleration_z_norm))+1, 1), "acceleration z"
  ) +
    geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - acceleration x y and z",
  ...
  plot_2var(
    ...
    labs(color = "acceleration"),
  # plot_3var(
  #   raw, "Raw - position x y and acceleration spatial",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$acceleration_spatial_norm, c(floor(min(raw$acceleration_spatial_norm)), floor(max(raw$acceleration_spatial_norm))+1, 1), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and acceleration spatial",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$acceleration_spatial_norm, c(floor(min(custom$acceleration_spatial_norm)), floor(max(custom$acceleration_spatial_norm))+1, 1), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and acceleration spatial",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$acceleration_spatial_norm, c(floor(min(custom_elevation$acceleration_spatial_norm)), floor(max(custom_elevation$acceleration_spatial_norm))+1, 1), "acceleration spatial"
  # ) +
  #   geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - acceleration planar and spatial",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    ...
    labs(color = "acceleration"),
  plot_2var(
    custom, "Custom - acceleration planar and spatial",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    ...
    labs(color = "acceleration"),
  plot_2var(
    custom_elevation, "Custom elevation - acceleration planar and spatial",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
...
ggsave("output/distance.png", grid.arrange(
  # plot_3var(
  #   raw, "Raw - position x y and distance spatial",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  #   raw$dist_spatial, c(0, floor(max(raw$dist_spatial))+1, 100), "distance spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and distance spatial",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  #   custom$dist_spatial, c(0, floor(max(custom$dist_spatial))+1, 100), "distance spatial"
  # ) +
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and distance spatial",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  #   custom_elevation$dist_spatial, c(0, floor(max(custom_elevation$dist_spatial))+1, 100), "distance spatial"
  # ) +
  #   geom_point(alpha = 1),
  plot_2var(
    raw, "Raw - distance planar and spatial",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    ...
    labs(color = "distance"),
  plot_2var(
    custom, "Custom - distance planar and spatial",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    ...
    labs(color = "distance"),
  plot_2var(
    custom_elevation, "Custom elevation - distance planar and spatial",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
...
ggsave("output/steering.png", grid.arrange(
  # plot_3var(
  #   raw, "Raw - position x y and steering",
  #   raw$position_x-min(raw$position_x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
  #   raw$position_y-min(raw$position_y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y",
  ...
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom, "Custom - position x y and steering",
  #   custom$position_x-min(custom$position_x), c(0, floor(max(custom$position_x-min(custom$position_x)))+1, 100), "position x",
  #   custom$position_y-min(custom$position_y), c(0, floor(max(custom$position_y-min(custom$position_y)))+1, 100), "position y",
  ...
  #   geom_point(alpha = 1),
  # plot_3var(
  #   custom_elevation, "Custom elevation - position x y and steering",
  #   custom_elevation$position_x-min(custom_elevation$position_x), c(0, floor(max(custom_elevation$position_x-min(custom_elevation$position_x)))+1, 100), "position x",
  #   custom_elevation$position_y-min(custom_elevation$position_y), c(0, floor(max(custom_elevation$position_y-min(custom_elevation$position_y)))+1, 100), "position y",
  ...
  plot_2var(
    ...
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom, "Custom - steering",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$steering, c(floor(min(custom$steering)), floor(max(custom$steering))+1, 1), "steering"
  ) +
    geom_line(color="#6B9EE1"),
  plot_2var(
    custom_elevation, "Custom elevation - steering",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$steering, c(floor(min(custom_elevation$steering)), floor(max(custom_elevation$steering))+1, 1), "steering"
  ) +
    geom_line(color="#6B9EE1"),
...
ggsave("output/distance reference.png", grid.arrange(
  plot_2var(
    road_raw, "Raw - distance from reference",
    road_raw$x-min(road_raw$x), c(0, floor(max(raw$position_x-min(raw$position_x)))+1, 100), "position x",
    road_raw$y-min(road_raw$y), c(0, floor(max(raw$position_y-min(raw$position_y)))+1, 100), "position y"
  ) +
    ...
    labs(color = "road"),
  plot_2var(
    road_custom, "Custom - distance from reference",
    road_custom$x-min(road_custom$x), c(0, floor(max(road_custom$x-min(road_custom$x)))+1, 100), "position x",
    road_custom$y-min(road_custom$y), c(0, floor(max(road_custom$y-min(road_custom$y)))+1, 100), "position y"
  ) +
    ...
    labs(color = "road"),
  plot_2var(
    road_custom_elevation, "Custom elevation - distance from reference",
    road_custom_elevation$x-min(road_custom_elevation$x), c(0, floor(max(road_custom_elevation$x-min(road_custom_elevation$x)))+1, 100), "position x",
    road_custom_elevation$y-min(road_custom_elevation$y), c(0, floor(max(road_custom_elevation$y-min(road_custom_elevation$y)))+1, 100), "position y"
  ) +
    ...
    labs(color = "road"),
  plot_2var(
    raw, "Raw - distance from reference",
    raw$time-min(raw$time), c(0, floor(max(raw$time-min(raw$time)))+1, 10), "time",
    raw$dist_reference, c(0, max(floor(max(raw$dist_reference))+1, lane_width_raw), 100), "distance from reference"
  ) +
    geom_line(color="#6B9EE1") +
  ...
  plot_2var(
    custom, "Custom - distance from reference",
    custom$time-min(custom$time), c(0, floor(max(custom$time-min(custom$time)))+1, 10), "time",
    custom$dist_reference, c(0, max(floor(max(custom$dist_reference))+1, lane_width_custom), 1), "distance from reference"
  ) +
    geom_line(color="#6B9EE1") +
  ...
  plot_2var(
    custom_elevation, "Custom elevation - distance from reference",
    custom_elevation$time-min(custom_elevation$time), c(0, floor(max(custom_elevation$time-min(custom_elevation$time)))+1, 10), "time",
    custom_elevation$dist_reference, c(0, max(floor(max(custom_elevation$dist_reference))+1, lane_width_custom_elevation), 1), "distance from reference"
  ) +
    geom_line(color="#6B9EE1") +
...
var.test(raw$dist_reference, custom$dist_reference, conf.level = 0.9999, alternative = c("two.sided"))
#test between custom and custom_elevation
var.test(custom$dist_reference, custom_elevation$dist_reference, conf.level = 0.9999, alternative = c("two.sided"))
#From the output it can be deduced for dist_reference given raw and custom
#- from the test of equality of variances, with 99.99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the variances are equal
#also for dist_reference given custom and custom_elevation
#- from the test of equality of variances, with 99.99% confidence the two variances can be considered different and also the high p-value (0.88) reinforces the decision to accept the hypothesis that the variances are equal.
...
#test between raw and custom
t.test(raw$dist_reference, custom$dist_reference, conf.level = 0.9999, alternative = c("two.sided"), mu=0, var.equal = F)
#test between custom and custom_elevation
t.test(custom$dist_reference, custom_elevation$dist_reference, conf.level = 0.9999, alternative = c("two.sided"), mu=0, var.equal = T)
#From the output it can be deduced for dist_reference given raw and custom
#- from the test of equality of variances, with 99.99% confidence the two variances can be considered different and also the high p-value (0.40) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for dist_reference given custom and custom_elevation
#- from the test of equality of variances, with 99.99% confidence the two variances can be considered different and also the high p-value (less than 0.87) reinforces the decision to accept the hypothesis that the means are equal.
...
#From the output it can be deduced for steering given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (0.04) reinforces the decision to reject with reservation the hypothesis that the means are equal
#also for steering given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (less than 0.77) reinforces the decision to accept the hypothesis that the means are equal.

#so the custom road generator influences the mean steering, but the elevation doesn't effect the steering.
...
#From the output it can be deduced for dist_planar given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (0.21) reinforces the decision to accept the hypothesis that the means are equal
#also for dist_planar given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered can be considered equal and also the high low p-value (0.56) reinforces the decision to accept the hypothesis that the means are equal.
...
#From the output it can be deduced for dist_spatial given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (0.21) reinforces the decision to accept the hypothesis that the means are equal
#also for dist_spatial given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered can be considered equal and also the high p-value (0.56) reinforces the decision to accept the hypothesis that the means are equal.
...
#From the output it can be deduced for speed_planar given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the low p-value (5.76e-3) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for speed_planar given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (0.83) reinforces the decision to decisively reject the hypothesis that the means are equal

#so the custom road generator influences the average speed 2D (also the variance), but the elevation doesn't effect the speed 2D.
...
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (0.95) reinforces the decision to accept the hypothesis that the variances are equal
...
#From the output it can be deduced for speed_spatial given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the low p-value (5.77e-3) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for speed_spatial given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (0.83) reinforces the decision to accept the hypothesis that the means are equal

#so the custom road generator influences the average speed 3D (also the variance), but the elevation doesn't effect the speed 3D.
...
#From the output it can be deduced for position_p given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the high p-value (0.98) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for position_p given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered different and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
...
#From the output it can be deduced for acceleration_z given raw and custom
#- from the test of equality of variances, with 99% confidence the two variances can be considered equal and also the low p-value (2.33e-4) reinforces the decision to decisively reject the hypothesis that the means are equal
#also for acceleration_z given custom and custom_elevation
#- from the test of equality of variances, with 99% confidence the two variances can be considered equal and also the extremely low p-value (less than 2.2e-16) reinforces the decision to decisively reject the hypothesis that the means are equal
